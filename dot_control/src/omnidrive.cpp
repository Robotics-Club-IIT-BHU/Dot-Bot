...
namespace omnidrive{
  ...
  drive::drive(ros::NodeHandle& nodeHandle):n(nodeHandle){
    ...
    joint_states=n.subscribe("/dot/joint_states", 1, &drive::onJointStateMessage, this);
    imu_sub = n.subscribe("/dot/imu",1, &drive::imuCallBack, this);
    ...
    file_storage << "velocity_l,velocity_r,velocity_b,calc_velocity_l,calc_velocity_r,calc_velocity_b" << std::endl;
  ...
  void drive::onJointStateMessage(const sensor_msgs::JointState::ConstPtr& input){
    ...
    // for (int i = 0; i < input->name.size(); i++) {
        // if (input->name[i].c_str()[0] == 'l') {
            theta_left_current = input->position[1];
            current_left_velocity=input->velocity[1];
        // } else if (input->name[i].c_str()[0] == 'b') {
            theta_back_current = input->position[0];
            current_back_velocity=input->velocity[0];
        // } else if (input->name[i].c_str()[0] == 'r') {
            theta_right_current = input->position[2];
            current_right_velocity=input->velocity[2];
        // }
    // }
    ...
    file_storage<<current_left_velocity<<","<<current_right_velocity<<","<<current_back_velocity<<","<<v_left<<","<<v_right<<","<<v_back<<odom_theta<< std::endl;
  ...
  void drive::publishVel(){
    ...
    double v1, v2, v3;
    v1 = -round((L * wp - (vmx / 2) - (sqrt3by2 * vmy))*50);
    v2 = -round((vmx + L * wp)*50);
    v3 = -round((L * wp - (vmx / 2) + (sqrt3by2 * vmy))*50);
    ...
    //ROS_INFO_STREAM_THROTTLE(2.0,"Lpos "<<lpos.data<<" Rpos "<<rpos.data<<" Bpos "<<bpos.data);
